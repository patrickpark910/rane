...
class Reactivity(MCNP_File):
    ...
    def process_rcty_keff(self):
        ...
        """
        self.keff_filename = f"{self.base_filename}_{self.run_type}"\
                                  f"_a{str(self.parameters['safe_height']).zfill(3)}"\
                                  f"_h{str(self.parameters['shim_height']).zfill(3)}"\
                                  f"_r{str(self.parameters['reg_height']).zfill(3)}_keff.csv"
        ...
        """
        self.h2o_temp_C  = float('{:.2f}'.format(self.h2o_temp_K - 273))
        self.uzrh_temp_C = float('{:.2f}'.format(self.uzrh_temp_K - 273)) # use 273 not 273
        if self.run_type == 'rcty_fuel':
            self.index_header, self.index_data = 'temp (C)', UZRH_FUEL_TEMPS_C
            self.row_val = self.uzrh_temp_C # make sure index_data and row_val use same units!
            ...
            self.index_header, self.index_data = 'void (at%)', H2O_VOID_PERCENTS
            self.row_val = self.h2o_void_percent
        ...
        if self.run_type == 'rcty_modr':
            df_keff.loc[self.row_val, "density"] = self.h2o_density
            df_keff.loc[self.row_val, "SaB lib"] = self.h2o_mt_lib
    ...
    def process_rcty_rho(self):
        ...
        self.rho_filename = f"{self.base_filename}_{self.run_type}"\
                                  f"_a{str(self.parameters['safe_height']).zfill(3)}"\
                                  f"_h{str(self.parameters['shim_height']).zfill(3)}"\
                                  f"_r{str(self.parameters['reg_height']).zfill(3)}_rho.csv"
    ...
    def process_rcty_coef(self):
        ...
        '''
        self.coef_filename = f"{self.base_filename}_{self.run_type}"\
                                  f"_a{str(self.parameters['safe_height']).zfill(3)}"\
                                  f"_h{str(self.parameters['shim_height']).zfill(3)}"\
                                  f"_r{str(self.parameters['reg_height']).zfill(3)}_coef.csv"
    ...
    def plot_rcty_coef(self):
        ...
        for rho_or_dollars in ['rho', 'dollars']:
            ...
            try:
                figure_filepath = self.results_folder+f"/{self.base_filename}_{self.run_type}"\
                                  f"_a{str(self.parameters['safe_height']).zfill(3)}"\
                                  f"_h{str(self.parameters['shim_height']).zfill(3)}"\
                                  f"_r{str(self.parameters['reg_height']).zfill(3)}"\
                                  f"_results_{rho_or_dollars}.png"
